...
| **E2** | Descrições com **“Use when…”** aumentam invocação da tool. | Rodada A: tools com descrições genéricas. Rodada B: descrições atuais do servidor. | Contagem manual de turnos em que o agente chama `search_instructions` / `get_instructions_batch` (anotar em uma folha de sessão). |
...
| **E4** | Embeddings (fase futura) superam fulltext em queries vagas. | Construir 20–30 queries reais da equipe; rotular documentos esperados. | Precisão@5 fulltext vs embeddings (quando implementado). |
...
- Documentar no README do servidor os **limites de tokens** implícitos (`max_results`, `max_chars_per_instruction` e teto total em `get_instructions_batch`).
...
**Aceite deste épico:** pelo menos **E1–E3** executados em 1 repo piloto com registro preenchido; ajustes aplicados ao servidor (descrições, defaults) com commit referenciado.
