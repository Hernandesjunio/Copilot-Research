...
└── tools
    ...
    │   ├── max_results: 1–10 (predefinição 5)
    ...
    │
    └── get_instruction
        ├── id ou path (relativo a INSTRUCTIONS_ROOT); um dos dois
        └── Conteúdo completo do corpo Markdown (após frontmatter); `max_chars` para truncar
            (predefinição 12000; teto máximo elevado no código).
```
...
3. Para texto integral, o cliente chama `get_instruction` com o `id` ou `path` devolvido na pesquisa.
...
**✅** O desenho combina resumo, excerto e lista ranqueada na resposta de **`search_instructions`**; o texto completo vem de **`get_instruction`**.
...
| **Corpo completo** (`content`) | Só em `get_instruction` | Completo (com truncagem opcional) |
...
Mensagens de aviso (`note`) podem aparecer quando a query está vazia ou não há matches. **`get_instruction`** devolve `content`, metadados e `truncated` se aplicável.
...
- **Nível 3 — Top-K:** `max_results` limita a lista (1–10; por omissão 5).
