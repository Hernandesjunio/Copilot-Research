...
| **E2** | Descrições com **“Use when…”** aumentam invocação da tool. | Rodada A: tools com descrições genéricas. Rodada B: descrições atuais do servidor. | Contagem manual de turnos em que o agente chama `search_instructions` / `get_instruction` (anotar em uma folha de sessão). |
