...
Recomendação inicial: **A + D** (repo central + variável de ambiente no registo MCP do VS), evoluindo para **B** ou **C** se precisarem de **pin** por versão em CI.

## Checklist para um novo repositório (~5 minutos)

1. Criar `.github/instructions/` (ou ficheiro único `copilot-instructions.md`) com **apenas** regras locais: idioma, segredos, boundaries do serviço, convênios que não podem falhar.
2. Incluir um parágrafo explícito: para padrões de arquitetura partilhados, **usar o MCP `corporate-instructions`** (`search_instructions` antes de desenhar soluções transversais).
3. Registar o servidor MCP no Visual Studio (ver [README do servidor](../../mcp-instructions-server/README.md)) com `INSTRUCTIONS_ROOT` correto.
4. (Opcional) Script `tools/sync-instructions.ps1` que faz `git pull` no repo canónico — para quem usa clone único.
...
- O processo MCP reconstrói o índice ao **arrancar**. Após `git pull` no corpus, **reiniciar** o servidor MCP no VS (ou o IDE) para ver alterações.
...
**Aceite deste épico:** playbook acordado pela equipa + 2–3 repos piloto migrados + instruções de onboarding para novos projetos.
