...
| `search_instructions` | Busca por intenção textual sobre o corpus, reutilizando o mesmo vocabulário de metadados e filtros do catálogo (`tags`, `tags_mode`, `kind`, `scope`, `priority`, `status`, `owner`, `workspace_evidence_required`) e acrescentando campos próprios de busca como `query`, `max_results`, `current_file_path`, `relevance` e multi-query (`queries` + `consolidated`). `current_file_path` é evidência contextual opcional para filtros declarativos como `applies_to`; não faz o servidor ler o workspace do utilizador. |
...
| `detect_instruction_conflicts` | Relações de conflito/precedência entre instructions com orientação para uso seguro pelo agente. |
