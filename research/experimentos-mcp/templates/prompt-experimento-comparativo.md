...
`docs/experimentos-mcp/Resultados/YYYY-MM-DD__NOME-DO-EXPERIMENTO__{{SUFIXO_ARQUIVO}}.md`
...
Use `YYYY-MM-DD` como a data de hoje. Em `NOME-DO-EXPERIMENTO`, use um slug curto (kebab-case). O sufixo final do arquivo identifica o cenário do experimento (`{{SUFIXO_ARQUIVO_LABEL}}`).

## Saída esperada

### Parte 1 — Execução (obrigatório)

#### {{PARTE1_ITEM_1_TITULO}}
{{PARTE1_ITEM_1_CORPO}}
...
- `decisao`
- `ancoragem` {{ANCORAGEM_DECISIONS_JSON}}
...
## Referência: placeholders para instantiar o baseline

Ao preencher os valores abaixo e remover esta secção, o texto deve coincidir com `prompt-sem-mcp-e-instructions.md`.

| Placeholder | Valor (baseline) |
|-------------|------------------|
| `{{CONTEXTO_DO_EXPERIMENTO}}` | Bloco completo começando em `## Contexto do experimento` até ao fim da lista de objetivos (inclusive linha em branco após a lista, como no ficheiro base). |
| `{{TAREFA_CORPO}}` | Texto desde `Implemente (codifique)...` até ao fim de `resiliência e tolerância a falhas.` |
| `{{REGRA_1_EXECUCAO}}` | Uma linha: `1. Execute a tarefa normalmente com base apenas no código disponível.` — **sem** newline final (o ficheiro do template já separa esta linha da regra `2.`). |
| `{{SUFIXO_ARQUIVO}}` | `baseline` |
| `{{SUFIXO_ARQUIVO_LABEL}}` | `baseline` (manter backticks no texto final: \`baseline\`) |
| `{{PARTE1_ITEM_1_TITULO}}` | `1) Ticket canônico (copiar no topo da resposta)` |
| `{{PARTE1_ITEM_1_CORPO}}` | Parágrafo introdutório + três bullets PUT/GET/integrações (como no ficheiro base). |
| `{{ANCORAGEM_DECISIONS_JSON}}` | `(codigo_repo \| inferencia)` |

### Outros cenários

- Podes alterar **apenas** os placeholders (e, se necessário, a primeira linha do prompt se o foco não for “ausência de contexto estruturado”).
- Mantém **inalteradas** as secções “Métricas de execução”, nomes dos blocos JSON, lista de campos e estrutura das secções 1–5, salvo acordo explícito para novo protocolo.

### Integridade vs ficheiro base

- Corre `python research/experimentos-mcp/templates/validate_prompt_experimento_comparativo.py` na raiz do repositório: instancia o template com texto tirado do próprio `prompt-sem-mcp-e-instructions.md` e confirma igualdade (newline final normalizado — o ficheiro base no disco pode não ter `\n` no EOF).
