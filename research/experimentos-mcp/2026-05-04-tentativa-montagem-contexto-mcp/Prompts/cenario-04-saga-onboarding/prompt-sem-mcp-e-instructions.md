...
**Identificação:** Cenário 4 (Saga onboarding) — condição **C: Baseline**.
...
- Fluxo **onboarding** em sequência: cliente → formulário → notificação (com mocks explícitos se não existirem integrações).
- **Estado persistido** da saga (tabela/store); **timeout** global; **retry** onde fizer sentido.
- **Compensações** na ordem inversa em falhas; **idempotência** em passos e compensações.
- **Correlação**/`SagaId` mínima alinhada ao projeto.
...
- Aplique as mudanças no código (orquestração manual, persistência de estado, compensações, idempotência conforme padrões do repo).
