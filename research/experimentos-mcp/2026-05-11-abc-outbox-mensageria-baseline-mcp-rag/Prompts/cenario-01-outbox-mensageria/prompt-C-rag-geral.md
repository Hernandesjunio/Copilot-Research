...
**Identificação:** Cenário 01 (Outbox / mensageria) — condição **C: RAG geral**.

Estou avaliando o comportamento do agente usando **RAG geral** sobre o workspace inteiro (incluindo `fixtures/`), comparado a um baseline sem fixtures e a um cenário MCP.
...
- **RAG geral (permitido):** é permitido recuperar/consultar qualquer ficheiro do workspace para reduzir inferência.
- **Controle experimental (obrigatório):** `fixtures/` deve permanecer **intacta** durante toda a execução.
- **Não usar** `.github/instructions/` como corpus normativo (fora do escopo do experimento).
- **MCP:** não é necessário aqui; se estiver ativo, não use como fonte principal (o foco é RAG geral do workspace).
...
`docs/experimentos-mcp/Resultados/YYYY-MM-DD__abc-outbox-mensageria-baseline-mcp-rag__rag.md`
