...
**Identificação:** Cenário 01 (Outbox / mensageria) — condição **A: Baseline (sem `fixtures/`)**.

Estou avaliando o comportamento do agente **sem contexto estruturado dedicado** e sem consulta a qualquer corpus externo.
...
- **Não usar** MCP `corporate-instructions` (nenhuma tool `corporate_instructions_*`).
- **Não usar** `.github/instructions/` ou qualquer outro corpus de policies.
- **Controle experimental (obrigatório):** durante esta execução, a pasta `fixtures/` deve estar **removida do workspace** (o experimentador move para fora e restaura ao final).
...
`docs/experimentos-mcp/Resultados/YYYY-MM-DD__abc-outbox-mensageria-baseline-mcp-rag__baseline.md`
