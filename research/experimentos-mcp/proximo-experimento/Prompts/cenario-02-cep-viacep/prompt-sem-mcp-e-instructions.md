...
**Identificação:** Cenário 2 (CEP / ViaCEP) — condição **C: Baseline**.
...
- Integração **ViaCEP** (ou equivalente) via `HttpClient` com políticas alinhadas ao projeto.
- Validação de **formato** de CEP e persistência de endereço ou estado de erro conforme modelo atual.
- **Resiliência:** timeout, retry/backoff, circuit breaker e fallback para validação pendente — valores/comportamentos demonstráveis.
- **Cache** por TTL e **observabilidade** mínima alinhada ao repo.
- Não quebrar o CRUD; coerência entre chamada HTTP externa e persistência explicada na implementação.
...
- Aplique as mudanças no código (validação de CEP, cliente HTTP, políticas de resiliência, cache, mapeamento de erros e modelo de dados conforme padrões do repo).
